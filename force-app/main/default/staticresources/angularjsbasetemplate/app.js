...
/*
 ...
 that.table.destroy();
 that.$e.empty();
 this.$e = $('#survey-targets-table-container').append('<table/>').find('table');
 this.$e.dataTable({
...
//setTimeout(showLogConsole, 2000);

    var app = angular
        .module('app', ['ngRoute', 'ngResource', 'ngCookies', 'ngSanitize', 'ngTouch', 'ui.bootstrap', 'templates-main'])
                ...
                .when('/data-audit-manager', {
                    ...
                    resolve: {
                        data: ['$q', 'AuditServices', function($q, AuditServices) {
                            ...
                            return AuditServices.getAuditResults()
                                .then(function(result) {
                                    return result.records;
                                })
                                .then(function(records) {

                                    var allObjectNames = _.map(records, function(r) { return r.BMP__Audit_Definition__r.BMP__Object_Name__c; });
                                    var uniqObjectNames = _.uniq(allObjectNames);
                                    ...
                                    return $q.all(promises)
                                        .then(function(results) {
                                            ...
                                            return {auditResults: records, metadata: metadata};
                                        });
    ...
    app.controller('HomeCtrl', ['$scope', '$location', '$route', function ($scope, $location, $route) {
        $location.path('survey-target-list');

        /*
         $scope.routes = _.map($route.routes, function(val, key) {return key;});
    ...
    app.provider('AuditServices', function AuditServicesProvider() {
        ...
        this.$get = ['$q', 'ForceTKClient', function ($q, ForceTKClient) {
            ...
            function client(cb) {
                ...
                if (!_client) {
                    ForceTKClient.client(ForceTKClient.isRunningOnSalesforce() ? null : {namedLoginParams: '2demo-admin'}).then(function (c) {
                        _client = c;
                        if (cb) cb(null, c);
                        delay.resolve(c);
                    }, function (err) {
            ...
            var obj = {
                ...
                getAuditResults: function() {
                    ...
                    return this.queryWrapper(soql, null);
                }

            };

            _.extend(obj, ForceTKClient.createWrappers(client));
    ...
    app.provider('FlagServices', function FlagServicesProvider() {
        ...
        var _baseSoql = 'select CreatedById, CreatedDate, End_Date_MRK__c, External_ID_MRK__c, Id, Active_Attachment_MRK__c, IsDeleted, IsLocked, LastModifiedById, LastModifiedBy.Name, Product_MRK__c,Product_MRK__r.Name, LastModifiedDate, LastReferencedDate, LastViewedDate, MayEdit, Name, OwnerId, Start_Date_MRK__c, SystemModstamp, (select Id, Name, ParentId, ContentType, BodyLength, OwnerId, CreatedDate, CreatedById, LastModifiedDate, LastModifiedById from Attachments), (select Id, Product_MRK__c, Product_MRK__r.Name from Product_Flags__r) from Flag_MRK__c';
        var _orderBy = 'order by LastModifiedDate desc';

        function productsSoql() {
            return "select Id, Name, Description_vod__c, External_ID_vod__c, Master_Product_Id_MRK__c from Product_vod__c where Product_Type_vod__c = 'Detail' and Active_MRK__c = true";
        }
        ...
        this.$get = ['$q', 'ForceTKClient', function ($q, ForceTKClient) {
            ...
            return {
                getProducts: function (cb) {
                    return queryWrapper(productsSoql(), cb);
                },
        ...
        function createFields(options) {
            var fields = [
                ...
                },
                {
                    name: 'AccountName',
                    display: 'Account',
                    path: 'Survey_Target_vod__r.Account_vod__r.Name',
                ...
                },
                {
                    ...
                    path: 'Survey_Target_vod__r.LastModifiedDate',
                    render: function (data, type, row, meta) {
    ...
    app.controller('FlagDetailCtrl', ['$scope', '$rootScope', '$routeParams', 'item', 'products', 'FlagServices', 'ForceTKClient', '$location', '$route', function ($scope, $rootScope, $routeParams, item, products, FlagServices, ForceTKClient, $location, $route) {
        ...
        $scope.saveItem = function (item) {
            ...
            ForceTKClient.client()
                .then(function (c) {
                    ...
                    if (!item.Id) {
                        c.create('Flag_MRK__c', item, hndlr, hndlr);
                    } else {
                        c.update('Flag_MRK__c', item.Id, item, hndlr, hndlr);
                    }
    ...
    app.controller('DataAuditManagerCtrl', ['$scope', '$sce', 'AuditServices', 'data', function($scope, $sce, AuditServices, data) {
