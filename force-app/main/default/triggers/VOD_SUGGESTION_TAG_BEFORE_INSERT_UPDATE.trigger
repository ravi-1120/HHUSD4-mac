trigger VOD_SUGGESTION_TAG_BEFORE_INSERT_UPDATE on Suggestion_Tag_vod__c (before insert, before update) 
{
    Map<String, Id> recordTypeIdByDevName = new Map<String, Id>();
    Map<Id, String> recordTypeNameById = new Map<Id, String>();
    List<RecordType> recordTypes = [SELECT Id, DeveloperName from RecordType where SobjectType = 'Suggestion_Tag_vod__c'];
    ...
    for (Integer i=0; i < Trigger.size; i++) {
        Suggestion_Tag_vod__c newRecord = Trigger.new[i];
        Suggestion_Tag_vod__c oldRecord = (Trigger.isInsert ? null : Trigger.old[i]);
