trigger VOD_MC_CYCLE_PLAN_TARGET_BEFORE_UPDATE_DELETE on MC_Cycle_Plan_Target_vod__c (before update, before delete) {
    ...
    for(MC_Cycle_Plan_Target_vod__c target: Trigger.old){
    ...
    for(Integer i = 0; i< Trigger.old.size(); i++){
        MC_Cycle_Plan_Target_vod__c oldPlan = Trigger.old[i];
        //if true, then lock the record
        if(cyclePlanMap.get(oldPlan.Cycle_Plan_vod__c).Lock_vod__c && !profileModAllData && !isMccpAdmin){
            if(Trigger.isDelete){
                oldPlan.addError(errorText);
            }
            else{
                MC_Cycle_Plan_Target_vod__c newPlan = Trigger.new[i];
