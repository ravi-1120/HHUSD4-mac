...
export default class Samplenew extends LightningElement {
    ...
    getBrandInformations() {
        getBrandInformations({ prodId: this.recordId, objectName: 'Meeting Request'})
            .then(result => {
    ...
    handleSectionToggle(event) {
        console.log("open handleSectionToggle");
    ...
    readmore3() {
        ...
        if (width < 768) {
            this.acr1 = true;
            this.acr2 = false;
            this.acrname = 'Indication' + ' for ' + this.prodname + ' ' + this.genericname;
            this.template.querySelector('.scroll_b_3')?.classList.remove('db');
            this.template.querySelector('.acc2')?.classList.remove('db');
