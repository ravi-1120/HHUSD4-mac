({
    fetchData: function (cmp, event, helper) {
        console.log('in fetchData of MSD_CORE_RelatedListHelper');
         try{
        ...
        action.setCallback(this, function(actionResult) {
            if(actionResult.getState() === "SUCCESS"){
                ...
                cmp.set('v.userProfile', res.userProfile);
            }
        });
        ...
        for(var i in columns){
            if(columns[i].label == 'Product Name'){
                if (userProfile !='MSD_CORE Contact Center - Read Only User'){
                formattedcolumns.push({label: 'Product Name', fieldName: 'LinkName', type: 'url', 
                                       typeAttributes: {label: { fieldName: 'MSD_CORE_Product_Name__c' }, target:'_self'}});
            }
                else{
                    formattedcolumns.push({label: 'Product Name', fieldName: 'MSD_CORE_Product_Name__c',  
                                       typeAttributes: {label: { fieldName: 'MSD_CORE_Product_Name__c' }, target:'_self'}});
                }
            }
        ...
        cmp.set("v.columns",formattedcolumns);
